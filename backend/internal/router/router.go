...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	dispositionAdviceRepository := repository.NewDispositionAdviceRepository(db)
	...
	dispositionAdviceService := service.NewDispositionAdviceService(dispositionAdviceRepository, securityService)
	...
	dispositionAdviceHandler := handler.NewDispositionAdviceHandler(dispositionAdviceService)
	systemHandler := handler.NewSystemHandler(securityService, bridgeAssetService, inspectionRoundService, defectFindingService, priorityDecisionService, dispositionAdviceService, db, redisClient)
	...
	dispositionAdviceHandler.Register(api)
