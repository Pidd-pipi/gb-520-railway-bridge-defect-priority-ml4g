...
type SystemHandler struct {
	...
	dispositionAdvice service.DispositionAdviceService
	db                *gorm.DB
	redis             *redis.Client
}

func NewSystemHandler(security service.SecurityService, bridgeAsset service.BridgeAssetService, inspectionRound service.InspectionRoundService, defectFinding service.DefectFindingService, priorityDecision service.PriorityDecisionService, dispositionAdvice service.DispositionAdviceService, db *gorm.DB, redisClient *redis.Client) *SystemHandler {
	return &SystemHandler{security: security, bridgeAsset: bridgeAsset, inspectionRound: inspectionRound, defectFinding: defectFinding, priorityDecision: priorityDecision, dispositionAdvice: dispositionAdvice, db: db, redis: redisClient}
}
...
func (h *SystemHandler) Overview(c *gin.Context) {
	...
	dispositionAdviceCounts, err := h.dispositionAdvice.LevelCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["dispositionAdvices"] = dispositionAdviceCounts
